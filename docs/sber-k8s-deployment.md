...
`JIRA_EMAIL`, optional `JIRA_USERNAME` и `JIRA_API_TOKEN`. Для Jira Server
`JIRA_USERNAME` может быть логином сервисного аккаунта, а `JIRA_API_TOKEN` -
его паролем или секретом. Keycloak/OIDC login пользователя нужен только для
входа в приложение; его пароль или token в Jira не пробрасываются. Jira
