...
JIRA_USERNAME=<optional-jira-server-login>
JIRA_API_TOKEN=<optional-token-or-password>
JIRA_PASSWORD=<optional-service-account-password>
JIRA_MAX_RESULTS=100
```

Jira REST API вызывается backend-контейнером через service account из
`JIRA_EMAIL`, optional `JIRA_USERNAME`, `JIRA_API_TOKEN` и optional
...
backend использует `JIRA_API_TOKEN` как пароль fallback. Keycloak/OIDC login
пользователя нужен только для входа в приложение; его пароль или token в Jira
не пробрасываются. Jira credentials не задаются через Admin Back Office.
