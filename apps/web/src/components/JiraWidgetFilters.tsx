...
import { jiraAnalyticsOperatorsFor } from "../app/jiraAnalytics";
...
function FilterEditor({ filter, fields, onChange, onDelete }: {
  ...
  return <div className="jira-widget-filter-row">
    ...
    <input value={filter.value} disabled={noValue} placeholder={filter.operator === "oneOf" || filter.operator === "noneOf" ? uiText("ui.jira.commaSeparatedValues") : undefined} onChange={(event) => onChange({ ...filter, value: event.target.value })} />
    <button type="button" className="icon-button danger" title={uiText("ui.jira.removeCondition")} onClick={onDelete}><Trash2 size={16} /></button>
  </div>;
}
...
export function JiraWidgetFilters({ widget, fields, onChange }: {
  ...
  const { t: uiText } = useInterfaceTranslation();
  const { jira: { jiraAnalyticsFilterLogicLabel } } = useJiraTranslations();
  ...
  return <fieldset className="jira-widget-filter-fieldset">
    <legend>{uiText("ui.jira.conditions")}</legend>
    <div className="jira-widget-filter-mode">
      <span id={filterLogicLabelId}>{uiText("ui.jira.conditionCombination")}</span>
      <div className="jira-widget-segments" role="group" aria-labelledby={filterLogicLabelId}>
        <button type="button" className={widget.filterLogic === "and" ? "active" : ""} aria-pressed={widget.filterLogic === "and"} onClick={() => onChange({ ...widget, filterLogic: "and" })}>{uiText("ui.jira.matchAllAnd")}</button>
        <button type="button" className={widget.filterLogic === "or" ? "active" : ""} aria-pressed={widget.filterLogic === "or"} onClick={() => onChange({ ...widget, filterLogic: "or" })}>{uiText("ui.jira.matchAnyOr")}</button>
      </div>
    </div>
    <div className="jira-widget-filter-list">
      {widget.filters.map((filter, index) => <div className="jira-widget-filter-item" key={filter.id}>
        {index > 0 ? <div className="jira-widget-filter-connector"><span className="jira-widget-filter-connector-value" aria-hidden="true">{filterConnector}</span><span className="jira-widget-filter-connector-label">{uiText("ui.jira.linkToPreviousCondition")} {filterConnector}</span></div> : null}
    ...
    <button type="button" className="secondary-button" onClick={() => onChange({ ...widget, filters: [...widget.filters, { id: uid("filter"), field: fields[0] ?? "issueKey", operator: "equals", value: "" }] })}><Plus size={16} />{uiText("ui.jira.condition")}</button>
  </fieldset>;
}
