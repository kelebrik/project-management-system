...
function WidgetEditor({ widget, aggregate, aggregates, onChange, onClose }: {
  ...
  return <aside className="jira-widget-editor" aria-label="Настройки виджета"><header><h3>Настройки виджета</h3><button type="button" className="icon-button" title="Закрыть" onClick={onClose}><X size={21} /></button></header>
    ...
    <label>Сортировка<select value={widget.sortBy} onChange={(event) => onChange({ ...widget, sortBy: event.target.value as JiraSemanticWidget["sortBy"] })}><option value="default">По умолчанию</option>{availableFields.filter((field) => ["goalDate", "issueKey", "eventAt", "durationHours", "commitCount", "mergeRequestCount", "sprintCount", "committedAt", "commitSha"].includes(field)).map((field) => <option key={field} value={field}>{JIRA_SEMANTIC_FIELD_LABELS[field]}</option>)}</select></label>
