...
function WidgetEditor({ widget, aggregate, aggregates, onChange, onClose }: {
  ...
  return <aside className="jira-widget-editor" aria-label="Настройки виджета"><header><h3>Настройки виджета</h3><button type="button" className="icon-button" title="Закрыть" onClick={onClose}><X size={21} /></button></header>
    ...
    <fieldset><legend>Поля</legend><div className="jira-aggregate-field-grid">{availableFields.map((field) => <label key={field} className="jira-aggregate-field-option"><input type="checkbox" checked={selected.has(field)} disabled={widget.selectedFields.length === 1 && selected.has(field)} onChange={(event) => { const selectedFields = event.target.checked ? [...widget.selectedFields, field] : widget.selectedFields.filter((item) => item !== field); const nextSet = new Set(selectedFields); if (!event.target.checked) setColumnWidthDraft(field, undefined); onChange({ ...jiraWidgetWithSelectedFields(widget, selectedFields), filters: widget.filters.filter((filter) => nextSet.has(filter.field)), dateField: widget.dateField && nextSet.has(widget.dateField) ? widget.dateField : null, groupBy: widget.groupBy !== "none" && !nextSet.has(FIELD_FOR_GROUP[widget.groupBy]!) ? "none" : widget.groupBy }); }} />{JIRA_SEMANTIC_FIELD_LABELS[field]}</label>)}</div></fieldset>
    {widget.visualization === "table" || widget.groupBy !== "none" ? <fieldset className="jira-widget-column-widths"><legend>Ширина колонок</legend><div>{widget.selectedFields.map((field) => <ColumnWidthControl key={field} widget={widget} field={field} draft={columnWidthDrafts[field] ?? String(jiraWidgetColumnWidth(widget, field))} onDraftChange={(value) => setColumnWidthDraft(field, value)} onCommit={() => commitColumnWidth(field)} onRevert={() => setColumnWidthDraft(field, undefined)} onReset={() => { const next = jiraWidgetWithoutColumnWidth(widget, field); setColumnWidthDraft(field, undefined); onChange(next); }} />)}</div></fieldset> : null}
    ...
    <fieldset><legend>Ширина виджета</legend><div className="jira-widget-segments"><button type="button" className={widget.width === "half" ? "active" : ""} onClick={() => onChange({ ...widget, width: "half" })}>1/2</button><button type="button" className={widget.width === "full" ? "active" : ""} onClick={() => onChange({ ...widget, width: "full" })}>1/1</button></div></fieldset>
  </aside>;
}
