...
import { ArrowLeft, ChevronLeft, ChevronRight, Download, Pencil, Plus, RefreshCw, Save, Settings2, Trash2, X } from "lucide-react";
...
function RecordsTable({ widget, result }: { widget: JiraSemanticWidget; result: SemanticResult }) {
  return <div className="jira-analytics-table-wrap"><table className="jira-analytics-table"><thead><tr>{widget.selectedFields.map((field) => <th key={field}>{JIRA_SEMANTIC_FIELD_LABELS[field]}</th>)}</tr></thead><tbody>{result.records.map((record) => <tr key={record.id}>{widget.selectedFields.map((field) => <td key={field}>{field === "issueKey" ? <a href={record.issueUrl} target="_blank" rel="noreferrer">{String(recordValue(record, field) ?? "-")}</a> : String(recordValue(record, field) ?? "-")}</td>)}</tr>)}</tbody></table></div>;
}
...
function WidgetContent({ widget, result, error, drilldown, onGroup, onBack }: {
  ...
  if (!result) return <div className="jira-widget-empty">Загрузка...</div>;
  if (drilldown) {
    return <><div className="jira-analytics-drilldown"><button type="button" className="icon-button" title="Вернуться к группам" onClick={onBack}><ArrowLeft size={17} /></button><strong>{drilldown.label}</strong><span>{result.totalRecords.toLocaleString("ru-RU")} строк</span></div><RecordsTable widget={widget} result={result} /></>;
  }
  ...
  return <div className="jira-analytics-number"><strong>{formatJiraAnalyticsMetric(widget.metric, result.value)}</strong><span>Строк: {result.totalRecords.toLocaleString("ru-RU")}</span></div>;
}
...
function WidgetEditor({ widget, aggregate, aggregates, onChange, onClose }: {
  ...
  const durationAvailable = selected.has("durationHours");
  const metrics = jiraAnalyticsMetrics.filter((metric) => metric === "count" || (metric.endsWith("Duration") && durationAvailable) || (metric === "commits" && selected.has("commitCount")) || (metric === "mergeRequests" && selected.has("mergeRequestCount")));
  const groups = jiraAnalyticsGroupings.filter((group) => group === "none" || selected.has(FIELD_FOR_GROUP[group]!));
  const dateFields = published.outputFields.filter((field) => field.type === "date" && selected.has(field.key)).map((field) => field.key);
  const listMode = widget.visualization === "table" && widget.metric === "count" && widget.groupBy === "none";
  return <aside className="jira-widget-editor" aria-label="Настройки виджета"><header><h3>Настройки виджета</h3><button type="button" className="icon-button" title="Закрыть" onClick={onClose}><X size={21} /></button></header>
    <label>Название<input value={widget.title} onChange={(event) => onChange({ ...widget, title: event.target.value })} /></label>
    <label>Агрегат<select value={widget.aggregateId} onChange={(event) => { const next = aggregates.find((item) => item.id === event.target.value); if (next) onChange({ ...defaultWidget(next, widget.placement), id: widget.id, title: widget.title }); }}>{aggregates.map((item) => <option key={item.id} value={item.id}>{item.published?.name ?? item.draft.name} · v{item.publishedVersion}</option>)}</select></label>
    <label>Результат<select value={listMode ? "list" : widget.metric} onChange={(event) => { const value = event.target.value; onChange(value === "list" ? { ...widget, metric: "count", groupBy: "none", visualization: "table" } : { ...widget, metric: value as JiraAnalyticsMetric, visualization: widget.groupBy === "none" ? "number" : "bar" }); }}><option value="list">Список строк</option>{metrics.map((metric) => <option key={metric} value={metric}>{JIRA_ANALYTICS_METRIC_LABELS[metric]}</option>)}</select></label>
    <fieldset><legend>Поля</legend><div className="jira-aggregate-field-grid">{availableFields.map((field) => <label key={field} className="jira-aggregate-field-option"><input type="checkbox" checked={selected.has(field)} disabled={widget.selectedFields.length === 1 && selected.has(field)} onChange={(event) => { const selectedFields = event.target.checked ? [...widget.selectedFields, field] : widget.selectedFields.filter((item) => item !== field); const nextSet = new Set(selectedFields); onChange({ ...widget, selectedFields, filters: widget.filters.filter((filter) => nextSet.has(filter.field)), dateField: widget.dateField && nextSet.has(widget.dateField) ? widget.dateField : null, groupBy: widget.groupBy !== "none" && !nextSet.has(FIELD_FOR_GROUP[widget.groupBy]!) ? "none" : widget.groupBy }); }} />{JIRA_SEMANTIC_FIELD_LABELS[field]}</label>)}</div></fieldset>
    ...
    <label>Сортировка<select value={widget.sortBy} onChange={(event) => onChange({ ...widget, sortBy: event.target.value as JiraSemanticWidget["sortBy"] })}><option value="default">По умолчанию</option>{widget.selectedFields.filter((field) => ["goalDate", "issueKey", "eventAt", "durationHours", "commitCount", "mergeRequestCount", "sprintCount"].includes(field)).map((field) => <option key={field} value={field}>{JIRA_SEMANTIC_FIELD_LABELS[field]}</option>)}</select></label>
