...
function stub(t: TestContext, target: unknown, key: string, replacement: Function) {
  ...
  let calls = 0;
  object[key] = (...args: unknown[]) => { calls++; return replacement(...args); };
...
test('automation filters historical data by project grants and business unit, admins bypass grants', async (t) => {
  ...
  const where: unknown[] = [];
  stub(t, prisma.project, 'findMany', async (args: { where: Record<string, unknown> }) => {
    where.push(args.where);
    return args.where.businessUnit ? [] : [p, { ...p, id: 'secret' }];
  });
  stub(t, prisma.projectAccess, 'findMany', async () => [{ projectId: 'p', level: 'VIEW' }]);
  stub(t, prisma.businessUnit, 'findFirst', async () => ({ id: 'unit' }));
...
test('unauthorized scenario requests cannot read the schedule', async (t) => {
  ...
  stub(t, prisma.project, 'findMany', async () => []);
  stub(t, prisma.projectAccess, 'findMany', async () => []);
  stub(t, prisma.businessUnit, 'findFirst', async () => ({ id: 'unit' }));
  const scheduleRead = stub(t, prisma.wbsItem, 'findMany', async () => { throw new Error('must not read'); });
  let status = 200;
