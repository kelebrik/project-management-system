...
async function ensureProjectReadAccess(
  ...
  prisma: PrismaClient,
  projectAccessLevel: typeof userProjectAccessLevel,
) {
  const project = await prisma.project.findUnique({
    where: { id: projectId },
    select: { id: true },
  });
  if (!project) return { ok: false as const, status: 404, error: 'Проект не найден' };
  ...
  if (await projectAccessLevel(user.id, projectId)) return { ok: true as const, user };
  return { ok: false as const, status: 403, error: 'Нет доступа к этому проекту' };
}
...
export function registerJiraAggregateRoutes(
  ...
  projectAccessLevel: typeof userProjectAccessLevel = userProjectAccessLevel,
) {
  router.get('/projects/:projectId/jira/aggregates', async (req, res) => {
    const access = await ensureProjectReadAccess(req.params.projectId, req, prisma, projectAccessLevel);
    if (!access.ok) {
